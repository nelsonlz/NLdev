...
$this->menu=array(
    ...
    array('label' => 'Imprimir Boleto','icon'=>'print', 'url' => '#', 'linkOptions'=>array('onclick'=>'javascript: trigger_click()')),
    array('label'=>'Enviar Boleto','icon'=>'envelope','url'=>array('update','id'=>$model->aLink)),
    array('label'=>'Deletar Boleto','icon'=>'remove','url'=>'#','linkOptions'=>array('submit'=>array('delete','id'=>$model->aLink),'confirm'=>'Are you sure you want to delete this item?')),
);
?>
...
<?php 
        ...
        // ----------- INSTRU��ES -------------------------------
            ...
            $aInstrucoes = explode("\n",$aTmp);
            if(count($aInstrucoes) > 0)
                $aDadosboleto['instrucoes1'] = $aInstrucoes[0];
            else
                $aDadosboleto['instrucoes1'] =" ";
